...
def main():
    ...
    with tab4:
        ...
        # Mapa Choropleth detalhado (apenas modo Demo - Florianopolis com shapefile local)
        if modo_demo:
            st.markdown("### 🗺️ Mapa Choropleth Detalhado - Bairros (Shapefile Local)")

            st.markdown("""
            <div class="insight-box">
            <b>Mapa Coropletico com Limites de Bairros do IBGE</b><br>
            Cada bairro e colorido de acordo com o preco medio dos imoveis na area.
            Cores mais quentes (amarelo/vermelho) indicam areas mais caras.
            Os limites sao do Censo 2022 do IBGE (shapefile local).
            </div>
            """, unsafe_allow_html=True)

            # Verificar se geopandas esta disponivel e se o shapefile existe
            import pathlib
            shapefile_path = pathlib.Path("SC_setores/SC_setores_CD2022.shp")

            if GEOPANDAS_AVAILABLE and shapefile_path.exists():
                try:
                    # Carregar shapefile dos setores censitarios de SC
                    @st.cache_data(ttl=3600)
                    def load_ibge_setores():
                        gdf = gpd.read_file(str(shapefile_path))
                        # Filtrar apenas Florianopolis (codigo IBGE: 4205407)
                        gdf_floripa = gdf[gdf['CD_MUN'] == '4205407'].copy()
                        # Converter para WGS84 (lat/lon)
                        gdf_floripa = gdf_floripa.to_crs(epsg=4326)
                        return gdf_floripa

                    gdf_setores = load_ibge_setores()

                    if len(gdf_setores) > 0:
                        # Usar NM_BAIRRO oficial do IBGE (nao os nomes do dataset)
                        # Filtrar setores que tem bairro definido
                        gdf_setores_com_bairro = gdf_setores[gdf_setores['NM_BAIRRO'].notna()].copy()

                        # Dissolver setores pelo bairro OFICIAL do IBGE
                        gdf_bairros = gdf_setores_com_bairro.dissolve(by='NM_BAIRRO', as_index=False)
                        gdf_bairros = gdf_bairros.rename(columns={'NM_BAIRRO': 'bairro'})
                        ...
                        # Spatial join - associar cada imovel ao seu BAIRRO OFICIAL
                        gdf_joined = gpd.sjoin(gdf_imoveis, gdf_bairros[['bairro', 'geometry']], how='inner', predicate='within')

                        # Calcular estatisticas por BAIRRO OFICIAL DO IBGE
                        bairro_stats_ibge = gdf_joined.groupby('bairro').agg({
                            'price': ['mean', 'median', 'count'],
                            'area_useful': 'mean'
                        }).reset_index()
                        bairro_stats_ibge.columns = ['bairro', 'price_mean', 'price_median', 'count', 'area_mean']

                        # Filtrar bairros com pelo menos 5 imoveis
                        bairro_stats_ibge = bairro_stats_ibge[bairro_stats_ibge['count'] >= 5]

                        # Merge geometrias com estatisticas
                        gdf_final = gdf_bairros.merge(bairro_stats_ibge, on='bairro', how='inner')

                        # Normalizar precos para cor
                        price_min = gdf_final['price_mean'].min()
                        price_max = gdf_final['price_mean'].max()
                        gdf_final['price_norm'] = (gdf_final['price_mean'] - price_min) / (price_max - price_min)
                        ...
                        # Funcao para cor baseada no preco
                        def get_color(price_norm):
                            if price_norm >= 0.8:
                                return '#FF0000'
                            elif price_norm >= 0.6:
                                return '#FF8C00'
                        ...
                        for idx, row in gdf_final.iterrows():
                            ...
                            <div style="font-family: Arial; width: 220px; background-color: #1E1E2E; padding: 10px; border-radius: 8px;">
                                ...
                                <p style="margin: 2px 0; color: #E0E0E0;"><b style="color: #FFFFFF;">Preco/m²:</b> R$ {row['price_mean']/row['area_mean']:,.0f}</p>
                            </div>
                            """

                            folium.GeoJson(
                                geojson_geom,
                                style_function=lambda x, pn=row['price_norm']: {
                                    'fillColor': get_color(pn),
                                    'color': '#FFFFFF',
                                    'weight': 1.5,
                                    'fillOpacity': 0.7
                                },
                                ...
                                tooltip=f"{row['bairro']}: R$ {row['price_mean']:,.0f}",
                                popup=folium.Popup(popup_html, max_width=250)
                            ).add_to(m)

                        # Calcular valores para a escala
                        price_20 = price_min + (price_max - price_min) * 0.2
                        price_40 = price_min + (price_max - price_min) * 0.4
                        price_60 = price_min + (price_max - price_min) * 0.6
                        price_80 = price_min + (price_max - price_min) * 0.8

                        # Formatar valores em milhares/milhoes
                        def format_price_legend(val):
                            if val >= 1_000_000:
                                return f"R$ {val/1_000_000:.1f}M"
                            else:
                                return f"R$ {val/1_000:.0f}k"

                        # Adicionar legenda com escala vertical continua (colorbar)
                        legend_html = f'''
                        <div style="position: absolute; top: 80px; right: 15px; z-index: 1000;
                            ...
                            <h4 style="margin: 0 0 10px 0; color: #4FC3F7; text-align: center; font-size: 12px;">Preco Medio</h4>
                            <div style="display: flex; align-items: stretch;">
                                ...
                                <div style="display: flex; flex-direction: column; justify-content: space-between;
                                    ...
                                    <span style="color: white; font-size: 10px; font-weight: bold;">{format_price_legend(price_max)}</span>
                                    <span style="color: #ccc; font-size: 9px;">{format_price_legend(price_80)}</span>
                                    <span style="color: #ccc; font-size: 9px;">{format_price_legend(price_60)}</span>
                                    <span style="color: #ccc; font-size: 9px;">{format_price_legend(price_40)}</span>
                                    <span style="color: #ccc; font-size: 9px;">{format_price_legend(price_20)}</span>
                                    <span style="color: white; font-size: 10px; font-weight: bold;">{format_price_legend(price_min)}</span>
                                </div>
                            </div>
                        ...
                        st.success(f"📊 **{len(gdf_final)} bairros** mapeados com limites agregados do IBGE (Censo 2022)")
                    else:
                        st.warning("Nenhum setor censitario encontrado para Florianopolis")

                except Exception as e:
                    st.error(f"Erro ao carregar dados do IBGE: {str(e)}")
                    st.info("Verifique se o arquivo SC_setores/SC_setores_CD2022.shp existe.")
            else:
                st.warning("⚠️ Dados do IBGE nao disponiveis. Instale geopandas ou baixe o shapefile.")

        # Mapa Choropleth para outras cidades de SC (nao demo)
        elif not modo_demo and estado_atual == 'Santa Catarina' and GEOPANDAS_AVAILABLE and len(df_geo) >= 3:
